...
router.post('/analyze/:username', async (req, res) => {
    try {
        ...
        try {
            ...
            const [rows] = await conn.query('SELECT id FROM profiles WHERE username = ?', [username]);
            ...
            if (topLanguages.length > 0) {
                const langValues = topLanguages.map(([lang, count]) => [profileId, lang, count]);
                await conn.query(
...
router.get('/profiles/:username', async (req, res) => {
    try {
        ...
        if (rows.length === 0) {
            return res.status(404).json({ success: false, message: 'Profile not found. Use POST /api/analyze/:username first.' });
        }
